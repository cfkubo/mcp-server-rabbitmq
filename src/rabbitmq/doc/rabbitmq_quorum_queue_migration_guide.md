You can migrate your classic mirrored queues to quorum queues on RabbitMQ brokers on version 3.13 or above by creating a new virtual host on the same cluster, or by migrating in place.

## Option 1: Migrating from classic mirrored queues to quorum queues with a new virtual host

You can migrate your classic mirrored queues to quorum queues on RabbitMQ brokers on version 3.13 or above by creating a new virtual host on the same cluster.
...
- Create a new policy in the new vhost. For recommendations on RabbitMQ policy configurations for quorum queues. Then, start the Federation you created earlier from the old vhost to the new vhost.
...
You can migrate your classic mirrored queues to quorum queues on RabbitMQ brokers on version 3.13 or above by migrating in place.
